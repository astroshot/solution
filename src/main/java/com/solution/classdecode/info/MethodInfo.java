...
public class MethodInfo {
    ...
    public void read(DataInputStream in) throws IOException {
        ...
        if (attributesCount > 0) {
            ...
            System.out.println("This method contains no attribute.");
        }
    }

    public String getFlags() {
        return MethodAccessInfo.getType(accessFlags);
    }

    public String getMethodName(ClassFileInfo classFileInfo) {
        ConstantUTF8Info utf8Info = (ConstantUTF8Info) classFileInfo.get(nameIndex);
        return utf8Info.getContent();
    }

    public String getMethodDescriptor(ClassFileInfo classFileInfo) {
    ...
    public String toString() {
        return String.format("Method access: %s Name index: %d Descriptor index: %d",
                getFlags(), nameIndex, descriptorIndex);
    }

    public String toString(ClassFileInfo classFileInfo) {
        return String.format("Method access: %s Name: %s Descriptor: %s",
                getFlags(), getMethodName(classFileInfo), getMethodDescriptor(classFileInfo));
    }
}
